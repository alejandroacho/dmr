...
def test_local_models_keep_container_dns_urls():
    """A model without a remote host must address its container by DNS name."""
    local_models = [m for m in ALL_MODELS if not m.is_remote]
    assert local_models, "expected at least one local model in the catalog"
    for model in local_models:
        assert model.base_url == f"http://{model.container_name}:{model.port}"
