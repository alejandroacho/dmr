...
#
# The container is named "ray-node-head" or "ray-node-worker". To open a shell
# inside a container after launch, use:
#       docker exec -it ray-node-head /bin/bash
#
# Then, you can execute vLLM commands on the Ray cluster as if it were a
# single machine, e.g. vllm serve ...
#
# To stop the container, use:
#       docker stop ray-node-head

# Check for minimum number of required arguments.
if [ $# -lt 4 ]; then
    echo "Usage: $0 docker_image head_node_ip --head|--worker path_to_hf_home [--detach] [additional_args...]"
    exit 1
fi
...
# Check for --detach flag among remaining args.
DETACH=false
FILTERED_ARGS=()
for arg in "$@"; do
    if [ "${arg}" == "--detach" ]; then
...
# Generate a deterministic container name based on node role.
if [ "${NODE_TYPE}" == "--head" ]; then
    CONTAINER_NAME="ray-node-head"
else
    CONTAINER_NAME="ray-node-worker"
fi

# In attached mode, define a cleanup routine that removes the container on exit.
...
if [ "${DETACH}" == "true" ]; then
    echo "Container running in background: ${CONTAINER_NAME}"
